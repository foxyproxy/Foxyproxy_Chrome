const afterInstallUrl = "http://getfoxyproxy.org/chrome/basic/install.html";
const afterUpdateUrl = "http://getfoxyproxy.org/chrome/basic/update.xml";
...
function Extension(){
	...
	var proxyList = $.map(JSON.parse(localStorage.getItem('proxyList')),  function (p){
			...
			{
				if(t.data.patterns[i].data.temp)
					t.data.patterns.splice(i,1);
				else
					i++;
			}
			return t;
	...
	this.applys = function(){
		...
		switch(this.state){
			case "disabled":
				...
				chrome.browserAction.setIcon({path : 'images/logo-disabled.png'});
				break;
			case "auto":
			...
			default:
				...
				for(var i=0;i<proxyList.length;i++)
					if(proxyList[i].data.id == this.state)
						proxy=proxyList[i];
				if(proxy)
				{
					console.log(proxy);
					chrome.browserAction.setIcon({imageData: IconCreator.paintIcon(self.icon, proxy.data.color)});
	...
	function checkFirst()
	{
		...
		if(!localStorage.getItem("installedtime")){
			...
			for(var i=0;i<a.length;i++)
				if(a[i].id === id)
					localStorage.setItem("version",a[i].version);
			});
			chrome.tabs.create({url: afterInstallUrl, selected: true });
			return true;
		}
		return false;
	}
	...
	this.drawIconAtRotation = function() {
	  ...
	  canvasContext.drawImage(self.currentIcon,
		  -Math.ceil(canvas.width/2),
		  -Math.ceil(canvas.height/2));
	...
	this.animateBlink = function(count, bInAction){
		if(!animating || bInAction){
			...
			if(count%2 == 0){
				...
				chrome.browserAction.setIcon({imageData:self.currentImageData});
			}
			if(count){
				setTimeout(function(){
	...
	self.settingsToXml = function () {
		...
		if(!self.settings.patternProxyQA || !proxyList[self.settings.patternProxyQA])
			quickAddNode.setAttribute("proxy-id","");
		else
			quickAddNode.setAttribute("proxy-id",proxyList[self.settings.patternProxyQA].data.id);
