var ProxyManager = {autoPacScriptPath:null, socksPacScriptPath:null, ProxyModes:{direct:"direct", manual:"manual", auto:"auto"}};
ProxyManager.directConnectionProfile = {id:"direct", name:"[proxy_directConnection]", proxyMode:ProxyManager.ProxyModes.direct, color:"inactive"};
ProxyManager.profileFromProxy = function(a) {
  var b = a.data.host + ":" + a.data.port;
  return{proxyMode:a.data.type, proxyHttp:a.data.isSocks ? null : b, proxySocks:a.data.isSocks ? b : "", socksVersion:a.data.socks, proxyConfigUrl:a.data.configUrl, proxyExceptions:"", useSameProxy:a.data.isSocks ? null : !0}
};
ProxyManager.profileAuto = function() {
  return{proxyMode:"auto", proxyHttp:"", proxySocks:"", socksVersion:"", proxyConfigUrl:"", proxyExceptions:"", useSameProxy:!1, isAutomaticModeProfile:!0}
};
ProxyManager.saveAutoPac = function() {
  var a = chrome.extension.getBackgroundPage().plugin, b = ProxyManager.generatePacAutoScript();
  try {
...
ProxyManager.apply = function(a) {
  var b = chrome.extension.getBackgroundPage().plugin, c = a.proxyMode == ProxyManager.ProxyModes.direct;
...
ProxyManager.saveSocksPacScript = function(a) {
  var b = chrome.extension.getBackgroundPage().plugin, a = ProxyManager.generateSocksPacScript(a);
  try {
...
ProxyManager.proxyToScript = function(proxy) {
  ...
  for(a = 0;a < b.length;a++) {
	...
	for(i in foxyProxy.localIps){
		  var d = b[a].data.url || "", f = b[a].data.type == "wildcard", e = b[a].data.whitelist != "Inclusive";
